...
| 2026-08-14 | EC2 fleet launch rejected as "instance type is not eligible for Free Tier"; later runs reached `qt6-pyside6` and failed resolving private RHI signatures such as `QRhiResourceUpdateBatch::updateDynamicBuffer(..., QByteArray)` | Runner selection was allowed to drift through the shared workflow; demoting private RHI declarations still left the RHI typesystem in Shiboken's load graph | Pin x86 and arm calls to GitHub-hosted `ubuntu-24.04` / `ubuntu-24.04-arm`; remove the all-private `typesystem_gui_rhi.xml` load from the local PySide6 patch. Chunk builds remain bounded at 270m inside their 360m job timeout so partial CAS salvage can run; the current remaining failures should be diagnosed from the next nightly after this change |
